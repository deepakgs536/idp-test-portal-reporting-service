...
class ReportingService:
    ...
    def update_test_report(self, test_id):
        ...
        section_wise_totals = {}
        for c in candidates:
            sec_scores = c.get("sectionScores", {})
        ...
        total_terminated = sum(1 for c in candidates if str(c.get("proctoringDetails", {}).get("status", "")).lower() == "terminated")
        total_completed = sum(1 for c in candidates if str(c.get("proctoringDetails", {}).get("status", "")).lower() == "success")
